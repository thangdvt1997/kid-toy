...
**Domain:** Vietnamese toy import/export B2C + B2B e-commerce platform (storefront + wholesale/dealer portal + import/export document management + multi-warehouse inventory)
**Researched:** 2026-09-21
**Confidence:** HIGH (core stack, ORM, payment libraries, i18n) / MEDIUM (MoMo/ZaloPay integration approach — no first-party NestJS SDKs exist) / LOW-MEDIUM (exact minor versions will drift by build time — pin via `npm info <pkg> version` at implementation time)
...
pnpm dlx create-turbo@latest kid-toy --package-manager pnpm

# Backend (apps/api)
pnpm add @nestjs/core @nestjs/common @nestjs/platform-express @nestjs/config
pnpm add @nestjs/passport passport passport-jwt passport-local bcrypt
...
pnpm add vnpay nestjs-vnpay
pnpm add @zalopay-oss/zalopay-nodejs   # verify maintenance status before committing; fallback = direct REST
pnpm add exceljs bwip-js qrcode sharp
pnpm add puppeteer                      # or pdfmake for simpler single-page docs
...
pnpm add -D @nestjs/testing jest ts-jest @types/node
...
**If PDF generation volume grows large (e.g., hundreds of POs/invoices/day):**
...
**If B2B order volume/catalog size grows past what Meilisearch's default config handles comfortably (very large catalogs, complex faceted search):**
...
**If the team later needs a native mobile app or exposes a dealer-facing public API:**
...
**If VPS resource constraints (RAM/CPU) become tight with all services co-located:**
...
---
*Stack research for: Vietnamese toy import/export B2C+B2B e-commerce platform*
*Researched: 2026-09-21*
